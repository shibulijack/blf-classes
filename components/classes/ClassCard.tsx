...
export function ClassCard({ cls }: ClassCardProps) {
  ...
  return (
    <Link href={`/classes/${cls.id}`} className="block">
      <div className="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-md transition-shadow active:scale-[0.98]">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              {cls.age_group && (
                <span className="text-xs text-gray-400">{cls.age_group}</span>
              )}
            </div>
            <h3 className="font-semibold text-gray-900 truncate">{cls.title}</h3>
            {cls.tutor_name && (
              <p className="text-sm text-gray-500 mt-0.5">by {cls.tutor_name}</p>
            )}
          </div>
